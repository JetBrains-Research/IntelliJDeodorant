...
public class CCFinderOutputParser extends CloneDetectorOutputParser {
    ...
    private static class Token {
        ...
        public String toString() {
            return String.format("<%s, %s>", getStartOffset(), getEndOffset());
        }
    }
    ...
    private Map<Long, Set<CloneFragment>> cloneFragments = new HashMap<>();
    private Set<Long> cloneSetIDs = new TreeSet<>(Long::compare);

    public CCFinderOutputParser(Project iJavaProject, String cloneOutputFilePath, String pathToCcfxprepdir) throws InvalidInputFileException {
        ...
        for (long cloneSetID : cloneSetIDs) {
            ...
            for (CloneFragment cloneFragment : cloneFragments) {
                try {
                    ...
                    int endOffset = tokens.get(cloneFragment.getEnd() - 1).getEndOffset() - 1;
                    CloneInstance cloneInstance = getCloneInstance(path, cloneInstanceIndex, startOffst, endOffset);
    ...
    private void readCloneSetData() throws InvalidInputFileException {
        ...
        try {
            ...
            while (true) {
                String line = readUtf8StringUntil(channel);
                ...
                if (line.length() == 0) {
                    line = readUtf8StringUntil(channel);
                    if (!"java".equals(line))
                        throw new InvalidInputFileException();
                    break;
                }
            ...
            // Read file info
            while (true) {
                String filePath = readUtf8StringUntil(channel);
            ...
            while (true) {
                String remarkText = readUtf8StringUntil(channel);
            ...
            while (position < channel.size()) {
                ...
                CloneFragment cloneFragmentLeft = new CloneFragment(cloneGroupID, sourceFilesList.get(leftFileIndex - 1),
                        leftFileBegin, leftFileEnd);
                CloneFragment cloneFragmentRight = new CloneFragment(cloneGroupID, sourceFilesList.get(rightFileIndex - 1),
                        rightFileBegin, rightFileEnd);
    ...
    private String readUtf8StringUntil(FileChannel c) throws IOException {
        ...
        while (true) {
            ...
            if (array[0] == CCFinderOutputParser.LINE_FEED_BYTE) {
                byte[] toReturnBytes = new byte[bytes.size()];
                for (int i = 0; i < bytes.size(); i++)
                    toReturnBytes[i] = bytes.get(i);
                return new String(toReturnBytes, StandardCharsets.UTF_8);
            }
            bytes.add(array[0]);
        }
